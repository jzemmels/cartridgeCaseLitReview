...
\citet{Zhang2020} applies the Density-Based Spatial Clustering of Applications with Noise (DBSCAN) algorithm to the features from the cell-based comparison procedure to determine if any clusters form amongst the per-cell estimated registration values.
...
[Discuss the benefit of diagnostics - cite Kathryn's LIME project, etc.].

[Narrow-in on visual diagnostics]
...
Tools exist to explain a variety of models, but much of the literature on "explainable" algorithms are focused on machine learning algorithms such as Random Forests or Multi-layer Neural Networks.
[More to say here?]
