...
The performance of ACES compares favorably to several other methods, such as logistic regressions on smaller feature sets, random forests, and some variants of previous Congruent Matching Cells methods [@song_proposed_2013;  @zhang_convergence_2021]. 
...
The definition of this ratio is given by Equation \@ref(eq:ratio).
...
Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:trainDataResults-html)', '\\@ref(tab:trainDataResults-latex)'))` and Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:testDataResults-html)', '\\@ref(tab:testDataResults-latex)'))` in the Appendix provide a numerical summary of these results.
...
Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:trainDataResults-html)', '\\@ref(tab:trainDataResults-latex)'))` summarizes the accuracy, true positive, and true negative rates based on the training data for the five binary classifier models.
...
```{r trainDataResults-latex,echo=FALSE,eval=knitr::is_latex_output(),fig.align='center'}
structure(list(`Feature Set` = structure(c(1L, 2L, 2L, 3L, 3L
    ...
    "97.23", "98.14", "\\textbf{98.3}"), `True Neg.` = c("90.43", 
    "97.15", "97.23", "98.14", "\\textbf{98.3}"), `True Pos.` = c("90.77", 
...
```{r trainDataResults-html,echo=FALSE,eval=knitr::is_html_output(),fig.align='center'}
structure(list(`Feature Set` = structure(c(1L, 2L, 2L, 3L, 3L
), .Label = c("$C_0$", "$C_0$~+~Registration", "All~ACES"), class = "factor"), 
    Model = structure(c(1L, 2L, 3L, 2L, 3L), .Label = c("Baseline", 
    "RF", "LR"), class = "factor"), Accuracy = c("90.47", "97.16", 
    "97.23", "98.14", "\\textbf{98.30}"), `True Neg.` = c("90.43", 
    "97.15", "97.23", "98.14", "\\textbf{98.30}"), `True Pos.` = c("90.77", 
    "97.18", "97.23", "98.14", "\\textbf{98.27}")), row.names = c(NA, 
-5L), class = c("tbl_df", "tbl", "data.frame")) %>%
  knitr::kable(caption = "Accuracy, True Positive, and True Negative rates based on the training data for the 13 binary classifier models. This table shows a numeric summary of the results shown in the Results section. We bold the largest values in each column for emphasis.",
               format = "html",
               escape = FALSE,
               align = c("l","l",
                         "r","r","r"),
...
Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:testDataResults-html)', '\\@ref(tab:testDataResults-latex)'))` summarizes the accuracy, true positive, and true negative rates based on the test data for the 13 binary classifier models. 

The true positive rates for the test data are noticeably lower to those for the training data summarized in Table `r knitr::asis_output(ifelse(knitr::is_html_output(), '\\@ref(tab:trainDataResults-html)', '\\@ref(tab:trainDataResults-latex)'))`, although the true negative rates are similar.

```{r testDataResults-latex,echo=FALSE,eval=knitr::is_latex_output(),fig.align='center'}
structure(list(`Feature Set` = structure(c(1L, 2L, 2L, 3L, 3L
), .Label = c("$C_0$", "$C_0$~+~Registration", "All~ACES"), class = "factor"), 
    Model = structure(c(1L, 2L, 3L, 2L, 3L), .Label = c("Baseline", 
    "RF", "LR"), class = "factor"), Accuracy = c("89.44", "96.91", 
    "96.98", "97.66", "\\textbf{97.68}"), `True Neg.` = c("89.64", 
    "97.11", "97.21", "\\textbf{97.87}", "97.81"), `True Pos.` = c("86.82", 
...
```{r testDataResults-html,echo=FALSE,eval=knitr::is_html_output(),fig.align='center'}
structure(list(`Feature Set` = structure(c(1L, 2L, 2L, 3L, 3L
), .Label = c("$C_0$", "$C_0$~+~Registration", "All~ACES"), class = "factor"), 
    Model = structure(c(1L, 2L, 3L, 2L, 3L), .Label = c("Baseline", 
    "RF", "LR"), class = "factor"), Accuracy = c("89.44", "96.91", 
    ...
    "94.22", "93.87", "94.74", "\\textbf{95.94}")), row.names = c(NA, 
-5L), class = c("tbl_df", "tbl", "data.frame")) %>%
  knitr::kable(caption = "Accuracy, True Positive, and True Negative rates based on the test data for the 13 binary classifier models. This table shows a numeric summary of the results shown in the Results section. We bold the largest values in each column for emphasis.",
               format = "html",
               escape = FALSE,
               align = c("l","l",
                         "r","r","r"),
