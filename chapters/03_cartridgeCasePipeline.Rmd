...
The performance of ACES compares favorably to several other methods, such as random forests on smaller feature sets, logistic regressions, decision trees, and some variants of previous Congruent Matching Cells methods [@song_proposed_2013;@Zhang2020]. 
...
As an alternative to defining Congruent Matching Cells, @Zhang2020 proposed using a clustering algorithm from @Ester1996 to determine the number of cells in agreement on a specific registration.
...
We first use the open-source FiX3P web application **[cite Talen Fisher]** to manually annotate the breech face impression region.
...
<!-- ```{r cellComparison,echo=FALSE,fig.align='center',fig.pos="htbp"} -->
<!-- knitr::include_graphics("images/cellBasedComparisonAlgorithm.png") -->
<!-- ``` -->
...
The final set of features we calculate are based on visual diagnostic tools described in Chapter 3 **[CITE]**.
...
We use 10-fold cross-validation repeated thrice [@caret] to train three binary classifiers based on a logistic regression, a classification tree [@rpart], and a random forest [@breiman;randomForest].
...
However, models that are trained to maximize accuracy on imbalanced data often exhibit a "preference" for classifying new observations as the majority class [@Fernndez2018], which in our case are non-matches.
...
An alternative optimization criterion commonly used for imbalanced data is to select the model that maximizes the area under the Receiver Operating Characteristic (ROC) curve, which measures the performance of a model under different threshold values [@James2013].
...
```{r trainTestAccuracy,echo=FALSE,fig.align='center',fig.cap='We summarize classification accuracy, true negative, and true positive rates for both the training and testing results, represented as gray and black points/lines respectively, for 13 binary classifier models. Our primary interest is the test data results, but visualizing the training data results allows us to assess the generalizability of the models after training. In the first row, we consider a classifier based on a single feature, the Cluster Indicator feature $C_0$, as a baseline. The remaining rows show results from training/testing classifiers based on a decision tree (CART), random forest (RF), and logistic regression (LR) under various feature sets and optimization critieria. The second and third rows show results based on a subset of seven features from the ACES feature set while the fourth and fifth rows show results using all 19 ACES features. We also compare results from optimizing either the overall Accuracy (third and fifth rows) or the area under the Receiver Operating Characterstic curve (AUC, second and fourth rows) during training.',fig.pos="htbp",out.width="\\textwidth"}
...
This is analogous to the classification rule used in @Zhang2020.
...
As discussed in the next section, we consider this subset of features to represent the features used in Congruent Matching Cells methods [@song_proposed_2013;Zhang2020].
...
```{r testProbs,fig.align='center',fig.cap='A dot plot of the predicted match probabilities for the non-match and match comparisons in the test set based on a logistic regression model. As we expect, the non-match comparisons tend to have a low match probability. However, we see that there are many matching comparisons that also have a low match probability.',fig.pos="htbp",out.width="\\textwidth"}
knitr::include_graphics("images/chapter4-images/testProbs_plt.png")
```

The left side of Figure \@ref(fig:testProbsByFirearm) shows the predicted match probabilities for the same matching test comparisons shown in Figure \@ref(fig:testProbs), but now separated by the 15 firearms comprising the test set.
We see that the firearm T has far more matching comparisons with low match probabilities compared to the other 14 test firearms.
This is further underscored by the right side of the Figure \@ref(fig:testProbsByFirearm), which shows the ratio of misclassifications to total comparisons for every pair of test firearms based on the same logistic regression model used in Figure \@ref(fig:testProbs).
The main diagonal shows the false negative misclassifications while the off-diagonal shows the false positives.
For clarity, we use blank tiles for comparisons where 0 misclassifications occurred.
We see that the false negative rate for firearm T of 45.9% far outweighs the error rates from other firearm pairs.
Further, the 161 false negative firearm T comparisons comprise about 71% of all of the 226 false negative test comparisons and 5% of the 3,181 matching test comparisons.
...
Upon visual inspection of the scans from firearm T, we noted a lack of consistent markings on their surfaces that is not shared with scans from the other test firearms.

```{r testProbsByFirearm,fig.align='center',fig.cap='(Left) A dot plot of the predicted match probabilities for the match comparisons in the test set based on a logistic regression model, separated by firearm. We see that firearm T has more matching comparisons with low match probabilities than the other test firearms. (Right) Misclassifications divided by total number of pairwise comparisons for each pair of test firearms based on the same logistic regression model. For clarity, we do not show comparisons with 0 misclassifications. We note that the proportion of misclassified matching comparisons from firearm T of 45.9\\% is much higher than that of other comparisons.',fig.pos="htbp",out.width="\\textwidth"}
...
Noting the log scale on which these points are plotted, we see that the cell-based similarities vs. differences ratio $\bar{r}_{cell}$ is the most important variable followed by the cluster size $C$ and the cell-based correlation $\overline{cor}_{\text{cell}}$.
Interestingly, we note that these top three features consist of one feature from each of the three feature groups defined in the Methods section.

```{r rfVarImpPlt,out.width="\\textwidth",echo=FALSE,fig.pos="htbp",fig.cap="Variable importance measures from fitting a random forest to the training data set, repeated 10 times under various random seeds. The top three features consist of one feature from each of the three feature groups defined in the Methods section. Points are plotted on a log scale and vertically jittered for visibility.",out.width=".9\\textwidth"}
knitr::include_graphics("images/chapter4-images/varImpPlt.png")
```

## Discussion

We use a $C_0$-based classifier as a baseline because it is analogous to the classification rule proposed in @Zhang2020.
Similarly, the cell-based registration features are based on the the same cell-based comparison procedure used in @song_proposed_2013 and summarized in \@ref(fig:cellComparison).
...
This is why we consider separate classifiers based on these features for the training and testing results shown in Figure \@ref(fig:testProbsByFirearm).
Table \@ref(tab:previousWorkComparison) summarizes the similarities between the ACES algorithm and the algorithms proposed in @Zhang2020 and @song_proposed_2013.
...
Table \@ref(tab:classifMethodComparison) shows the test classification error rates of the Congruent Matching Cells (CMC) algorithm proposed in @song_proposed_2013, the $C_0$-based classifier like the one proposed in @Zhang2020, and the two ACES logistic regression models selected to balance the true negative and true positive rates and maximize the classification accuracy.
...
There is precedent for random forest models to be robust classifiers for forensic evidence [@hare_automatic_2016;@Park2019].
...
Compared to predominant algorithms like the CMC algorithm described in @song_proposed_2013, the ACES algorithm reduces the classification error rate by over 75% on our test data set.
...
The results in this paper were obtained using R 4.2.2. [@Rlanguage]
                                                                                                                                                                                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                                                                                                                                                                                               "99.50", "97.33", "98.93", "98.25", "\\textbf{99.56}"), `True Positive` = c("97.32", 
                                                                                                                                                                                                                                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                           "95.91", "97.32", "93.59", "98.23", "96.50"), `True Negative` = c("94.37", 
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             "\\textbf{99.90}", "97.33", "99.53", "98.25", "\\textbf{99.90}")), class = "data.frame", row.names = c(NA, 
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    -13L))%>%
  select(`Feature Set`,Criterion,Model,Accuracy,`True Negative`,`True Positive`) %>%
